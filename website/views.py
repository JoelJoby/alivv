    ...
    if request.method == 'POST':
        ...
        if email:
            if Subscriber.objects.filter(email=email).exists():
                messages.warning(
                    request, "You are already subscribed with this email!")
            else:
...
def home(request):
    top_categories = Category.objects.filter(
        is_top_category=True).order_by('priority')[:3]
    top_products = Product.objects.filter(
        is_top_product=True).order_by('priority')[:6]
...
def category(request, cat):
    ...
    categories_with_slugs = [(c.name, c.get_url_name())
...
def add_to_cart(request, pk):
    if request.method == 'POST':
        ...
        messages.success(
            request, f"{product.name} ({size}) added to cart successfully!" if size else f"{product.name} added to cart successfully!")
...
def checkout(request):
    if request.method == 'POST':
        ...
        if selected_address_id:
            try:
                address = CustomerDetails.objects.get(
                    id=selected_address_id, customer=customer)
            ...
            if not (country_id and address_line_1 and city and postcode):
                messages.error(
                    request, "Please fill all required address fields.")
                return redirect('checkout')

            country_inst = Country.objects.filter(id=country_id).first()
            state_inst = State.objects.filter(
        ...
        if country_id and address_line_1 and city and postcode:
            ...
            if address_id:
                try:
                    ...
                    address.save()
                    messages.success(
                        request, "Address details updated successfully.")
                except CustomerDetails.DoesNotExist:
                ...
                messages.success(
                    request, "Address details added successfully.")
...
def edit_address(request, pk):
    try:
        address = CustomerDetails.objects.get(
            id=pk, customer__email=request.user.email)
        ...
        if address.country:
            states = State.objects.filter(
                country=address.country).order_by('name')
